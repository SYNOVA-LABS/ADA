...
from vosk import Model, KaldiRecognizer # Vosk speech recognition library to recognize speech
...
def listen_for_question() -> str | None:
    ...
    try:
        ...
        with sd.RawInputStream(
            ...
            while time.time() - start_time < timeout: # loop for a maximum of 10 seconds (timeout)
                try:
                    data = q.get(timeout=2) # wait for audio data for 2 seconds before checking again meaning if we get any data in 2 seconds we flag that as a question an continue listening to that question
                    ...
                    partial = json.loads(rec.PartialResult()) # get partial result which is the text that was recognized so far during the current 2 second period
                    if partial.get("partial", "") == "" and full_text: # if we have silence for 2 seconds and some text thats means the user is done speaking
                        return full_text.strip() # return the full text as thast the final result
